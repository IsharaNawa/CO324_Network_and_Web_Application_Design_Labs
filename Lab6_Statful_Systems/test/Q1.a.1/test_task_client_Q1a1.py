import sys
sys.path.append('./src/Q1.a.1/')
from lab06_task_server_Q1a1 import *
...
def test_addTask() -> None:
    ...
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        for i in range(10):
            ...
            status_response = stub.getServerStatus(empty_pb2.Empty())
            task_history = list(status_response.tasks[response.id].pending)
            assert len(task_history) == 1
            assert type(task_history) == list, "Save added tasks as a list for each key. {1: [Task1], 2: [Task2], ...}"
        try:
...
def test_editTask():
    ...
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        for i in range(5):
            ...
            edit_response = stub.nondestructive_editTask(tasklist_pb2.Task(id=add_response.id, description="Edited task"))
            ...
            status_response = stub.getServerStatus(empty_pb2.Empty())
            task_history = list(status_response.tasks[edit_response.id].pending)
            assert len(task_history) == 2
            assert type(task_history) == list 
            assert task_history[0].id == task_history[1].id, "Save added tasks as a list for each key. {1: [Task1, Task1_edited], 2: [Task2, Task2_edited], ...}"
            assert task_history[1].description == "Edited task"
        ...
        for i in range(1):
            ...
            try:
                desc = random_string_generator(2000, string.ascii_letters)
                response = stub.nondestructive_editTask(tasklist_pb2.Task(id=existing_id, description=desc))
