...
class TaskapiImpl:
    ...
    def __enter__(self):
        ...
        with open(self.taskfile, mode="rb") as t:
            ...
            self.tasks: Mapping[int, List[tasklist_pb2.Task]] = {item.id: [item] for item in tasklist.pending}
            print(self.tasks)
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        with open(self.taskfile, mode="wb") as t:
            tasks = tasklist_pb2.Tasks(pending=[item[-1] for item in self.tasks.values()])
    ...
    def addTask(self, request: wrappers_pb2.StringValue, context) -> tasklist_pb2.Task:
        ...
        with self.ExLock:
            ...
            #since we need to implement non destructive method, we need to do make a dictionary with key as the id and value as the list
    ...
    def listTasks(self, request: tasklist_pb2.TaskQuery, context) -> tasklist_pb2.Tasks:
        ...
        return tasklist_pb2.Tasks(pending=[item[-1] for item in self.tasks.values()])

    def nondestructive_editTask(self, request: tasklist_pb2.Task, context) -> tasklist_pb2.Task:
        ...
        with self.ExLock:
            self.tasks[request.id].append(request)  #adding the new task to the list with the same id
            return request                          # returning the edited task

    def getServerStatus(self, request, response):
        out = {key: tasklist_pb2.Tasks(pending=self.tasks[key]) for key in self.tasks}
        return tasklist_pb2.ServerStatus(tasks=out)


TASKFILE = "./src/Q1.a.1/tasklist.protobuf"
